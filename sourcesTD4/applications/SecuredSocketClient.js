...
var options = {
    ca: FileSystem.readFileSync(Constants.PATH_PUBLIC_CERT)
}

var socket = TLS.connect(serverPort, serverHost, options, function () {
    SocketUtils.emit(socket, 'message', Message.getAuthMessage(senderName, password), true)
...
socket.on("data", function(data) {
    ...
    if (data.action === Message.MESSAGE_TYPE_SERVER_LIST_CLIENTS) {
        ...
        data.clients.forEach(function (client) {
            console.info('Client Id: ' + client.clientId + ' - Nick name: ' + client.nickName)
        })
    }
...
terminal.on('line', function (line) {
    ...
    if (!Utils.isEmpty(command)) {
        ...
        if (msgObject !== null) {
            SocketUtils.emit(socket, 'message', msgObject, true)
