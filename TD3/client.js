...
function chat_command(cmd, arg) {
    switch (cmd) {
        ...
        case 'nick':
            ...
            nick = arg;
            socket.emit('send', { type: 'notice', message: '', oldname: oldnick, newname: nick });
            break;
...
rl.on('line', function (line) {
    ...
    if (line[0] == "/" && line.length > 1) {
        ...
        socket.emit('send', {sender: sender_name, message: message_content, action: 'list'});
        rl.prompt(true);
    } else if (line == "q;"){
    ...
    //sender_name creates the group group_name -> cg;group_name
        ...
        if(line.length > 3){
            ...
            socket.emit('send',{sender: sender_name, group: group_name, action:'cgroupe', message: ''});
            } 
        } 
        if (group_name == null){
...
socket.on('message', function (data) {
    ...
    // Notice is sent by server (TD1)
        ...
        data.message = data.oldname + " changed its name to " + data.message;
        ...
        console.log("Number of clients: " + data.message);
