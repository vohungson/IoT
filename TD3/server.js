...
var listOfGroups = [];
var client_message = "";
var client_message1 = "";//this line for test case
    ...
    if(table == 'history'){ //read the table history
        db.each(`SELECT * FROM history`, (err, row) => {
            ...
            if(read_group == row.group_name){
                ...
                client_message1 += row.message;//this line for test case
            }
        });
...
io.sockets.on('connection', function (socket) {
    socket.on('send', function (data) {
        ...
        if(data.action == 'connection'){
            ...
            data.message = '' + listOfClients.length;
            io.sockets.emit('message', data);//send back number of clients to the client.
            ...
            data.message = data.newname;
            io.sockets.emit('message',data);//send back new name to the client.
        ...
        //{sender: sender_name, group: group_name ,action:'cgroupe'}
            ...
            data.message = data.group;
            io.sockets.emit('message',data);//send back to the client for test case! 
        }
        ...
        //{sender: sender_name ,group: 'group_name', action:'members'}
            ...
            if (!existGroup(data.group)){
                ...
                data.msg += Group.getList(group).length;//save the number of clients in the group
            }
            io.sockets.emit('message',data);
        }
        //{name: sender_name ,action:'groups'} -> groups;
            ...
            console.log(client_message);
            data.message = Group.getName(group);//for test case -> show the last group
            io.sockets.emit('message',data);
        } 
        //{sender: sender_name,group: 'group_name' ,action:'leave'} -> leave;group_name
            ...
            if (!existGroup(data.group)){
                ...
                if(clientExistInGroup(group,data.sender)){
                    ...
                    data.message = data.sender;//this line for test case
                } else {
            ...
            if(existClient(data.dest) && existGroup(data.group)){
                ...
                if(!clientExistInGroup(group,data.dest)){
                    if(!clientExistInBannedGroup(group,data.dest)){
                        ...
                        data.message = data.dest;//this line for test case
                    } else {
        ...
        //{sender: sender_name , group: 'group_name', dest: receiver_name, reason: 'reason', action:'ban'}
            ...
            if(existClient(data.dest) && existGroup(data.group)){
                ...
                data.message = data.dest;//this code for test case
            } else {
        ...
        //{sender: sender_name ,group: 'group_name', action:'msgs'}
            ...
            if(existGroup(data.group)) {
                ...
                setTimeout(() => {
                    data.msg += client_message;
                    data.message = client_message1;//this line for test case
                    io.sockets.emit('message',data);
                }, 500);//wait in 0.5s
            } else {
        ...
        //{sender: sender_name , group: 'group_name', action:'states'} -> states;group_name
            ...
            if(existGroup(data.group)) {
                ...
                setTimeout(() => {
                    data.msg += client_message;
                    data.message = 'okay';//this line for test case
                    io.sockets.emit('message',data);
                }, 500);//wait in 0.5s
            } else {
                data.msg += "SORRY - Group doesn't exist";
...
//The code for test cases of server
module.exports = {
    runServer: function(){
        return 'server';
    }
