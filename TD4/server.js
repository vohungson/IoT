...
server.setPrivateKey(
    crypto.createHash('sha256').update('server', 'utf8').digest()
);
...
db.serialize(() => {//create the table of history and the table of event
    ...
    db.run('CREATE TABLE pass(sender text NOT NULL, password text NOT NULL)');
});
...
function writePasswordIntoDatabase(sender, password){
    var exist_Sender = existSender(sender);
    if(!exist_Sender){
        sql = "INSERT INTO PASS (sender, password) VALUES('" 
            + sender + "','" 
            + password + "')";
        console.log(sql);
        db.run(sql, function(err) {
        ...
        listOfSenders.push(sender);
    } else {
        sql = "UPDATE pass SET password = '" + password 
                        + "' WHERE sender = '" + sender + "'" ;
        console.log(sql);
        db.run(sql, function(err) {
            if (err) {
...
function readPasswordFromDatabase(sender){
    db.each(`SELECT * FROM pass`, (err, row) => {
        if (err){
          throw err;
        }
        if(sender == row.sender){
            client_message = row.password;//save pass here and send it to client
        }
    });
}
...
function existSender (sender_name){
    var checkExistSender = false;
    for (var i = 0; i < listOfSenders.length; i ++){
        if(sender_name == listOfSenders[i]){
            checkExistSender = true;
            break;
        }
    }
    return checkExistSender;
}
...
io.sockets.on('connection', function (socket) {
    socket.on('send', function (data) {
        ...
        if(data.action == 'connection'){
            if(existClient(data.sender)){
                console.log(data.sender + " existed!");
                data.action = 'quit';
                socket.emit('message', data);
            } else {
        ...
        //TD4
        else if (data.action == 'register'){//write password into the database
            writePasswordIntoDatabase(data.sender,data.password); 
            setTimeout(() => {
                readPasswordFromDatabase(data.sender); 
                data.password = client_message;
            }, 500);//wait in 0.5s
                      
        } else if (data.action == 'secure'){//get public key from the client
            privateKey = server.computeSecret(data.publicKey, null, 'hex');//save private key 
            var json = {sender: data.sender, publicKey: server.getPublicKey(), action: data.action};
            console.log("Created server private key");
            io.sockets.emit('encryption', json);//send public key json to client
        } else if (data.action == 'securemessage'){//get message from the client and decrypt it
            console.log("Start decrypting message");
            var decipher = crypto.createDecipher(algorithm,privateKey);
            var decrypted = decipher.update(data.message,'hex','utf8');
            decrypted += decipher.final('utf8');
            console.log("Decrypted message is : " + decrypted);

            //send back secure message to client
            var cipher = crypto.createCipher(algorithm, privateKey);
            var secure_message = decrypted + " is secured!";
            console.log("Server send back this message: " + secure_message);
            let encryptMessage = cipher.update(secure_message, 'utf8', 'hex');
            encryptMessage += cipher.final('hex');
            var json = {sender: data.sender, message: encryptMessage, action:"clientdecrypt"};
            io.sockets.emit('encryption', json);
        } 
